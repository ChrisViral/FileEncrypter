...
public class ProtectorIntegrationTests
{
    ...
    public async Task MixedFiles_WithModeRestriction_FailureCount()
    {
        ...
        await File.WriteAllBytesAsync(plainPath, "secret"u8.ToArray());
        ...
        await File.WriteAllBytesAsync(sourceToEncryptPath, "keep"u8.ToArray());
        ...
        Result encResult = await encryptProtector.ProtectFile(new FileInfo(sourceToEncryptPath), CancellationToken.None);
        ...
        Result result = await protector.ProtectAll(new[] { new FileInfo(plainPath), new FileInfo(alreadyEncryptedPath) });
    ...
    public async Task CustomEncryptedExtension_ShouldEncryptAndDecryptCorrectly()
    {
        using TempDirectory tempDir = new();
        string plainPath = Path.Combine(tempDir.DirectoryPath, "plain.txt");
        await File.WriteAllBytesAsync(plainPath, "content"u8.ToArray());
        ...
        Result encResult = await encryptor.ProtectAll(new[] { new FileInfo(plainPath) });
        encResult.IsSuccess.Should().BeTrue();
        ...
        Result decResult = await decryptor.ProtectAll(new[] { new FileInfo(encryptedPath) });
        decResult.IsSuccess.Should().BeTrue();

        File.Exists(plainPath).Should().BeTrue();
        File.Exists(encryptedPath).Should().BeFalse();

        byte[] originalBytes = await File.ReadAllBytesAsync(plainPath);
    ...
    public async Task ProtectDirectory_AllDirectories_EncryptAllMatchingFiles()
    {
        ...
        await File.WriteAllBytesAsync(rootTxtPath, "root"u8.ToArray());
        ...
        await File.WriteAllBytesAsync(subTxtPath, "sub"u8.ToArray());
        string subLogPath = Path.Combine(subdir.FullName, "log.log");
        await File.WriteAllBytesAsync(subLogPath, "logdata"u8.ToArray());
        ...
        await File.WriteAllBytesAsync(nestedTxtPath, "nest"u8.ToArray());
        ...
        Result result = await protector.ProtectDirectory(new DirectoryInfo(tempDir.DirectoryPath));
